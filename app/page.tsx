...
function BarberCalendarCore() {
  ...
  // Native iOS-like day paging (horizontal scroll-snap) inside the Day Editor BODY.
  // This avoids JS gesture direction fights with vertical scrolling on iPhone.
  const dayPagerRef = useRef<HTMLDivElement | null>(null);
  const dayPagerSettleTimerRef = useRef<number | null>(null);
  const dayPagerProgrammaticRef = useRef(false);

  const centerDayPager = () => {
    const el = dayPagerRef.current;
    if (!el) return;
    const w = el.clientWidth || 1;
    dayPagerProgrammaticRef.current = true;
    el.scrollTo({ left: w, behavior: 'auto' });
    requestAnimationFrame(() => {
      dayPagerProgrammaticRef.current = false;
    });
  };
  ...
  const onDayPagerScroll = () => {
    const el = dayPagerRef.current;
    if (!el) return;
    if (dayPagerProgrammaticRef.current) return;
    if (dayPagerSettleTimerRef.current) window.clearTimeout(dayPagerSettleTimerRef.current);
    dayPagerSettleTimerRef.current = window.setTimeout(() => {
      const w = el.clientWidth || 1;
      const page = Math.round(el.scrollLeft / w); // 0=prev, 1=current, 2=next
      if (page === 0) shiftSelectedDay(-1);
      if (page === 2) shiftSelectedDay(1);
      // Re-center to the middle page instantly (native feel).
      dayPagerProgrammaticRef.current = true;
      el.scrollTo({ left: w, behavior: 'auto' });
      requestAnimationFrame(() => {
        dayPagerProgrammaticRef.current = false;
  ...
  useEffect(() => {
    // When the day editor opens or the selected day changes, keep the pager centered.
    if (!selectedDate || !selectedDayISO) return;
    centerDayPager();
    return () => {
      if (dayPagerSettleTimerRef.current) {
        window.clearTimeout(dayPagerSettleTimerRef.current);
        dayPagerSettleTimerRef.current = null;
      }
    };
          ...
          >
            <div className="flex h-full flex-col">
              ...
              <div className="mt-4 flex-1 overflow-hidden">
                ...
                >
                  <div style={{ flex: '0 0 100%', scrollSnapAlign: 'center' }} aria-hidden="true" />
